...
import QRCode from 'qrcode'; // ✅ Import ajouté
...
export default function QRModal({
  ...
  const [qrDataUrl, setQrDataUrl] = useState<string | null>(null); // ✅ État QR
  const [qrError, setQrError] = useState<string | null>(null);

  // 🔹 Génération QR quand le modal s’ouvre
  useEffect(() => {
    if (isOpen) {
      setQrDataUrl(null);
      setQrError(null);
  ...
  return (
    <AnimatePresence>
      ...
      <motion.div
        ...
        onClick={onClose}
      >
        {/* 🔹 Bulles animées — avec clé unique */}
        <div className="absolute inset-0 overflow-hidden pointer-events-none">
          {[...Array(12)].map((_, i) => (
            <motion.div
              key={`bubble-${i}`} // ✅ Clé unique garantie
            ...
            <div className="p-6 text-center">
              ...
              <div className="flex justify-center mb-5">
                {qrError ? (
                  ...
                  <motion.img
                    key={profileUrl} // ✅ Clé stable pour éviter le re-render inutile
                    src={qrDataUrl}
