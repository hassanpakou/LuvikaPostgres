...
const profileWithVisibility: PublicProfile = {
  ...
  // 🔹 Realtime sécurisé - Amélioré avec gestion d'erreurs et reconnexion
  useEffect(() => {
    const supabase = createClient();
    
    // 🔹 Canal pour les configurations de carte
    const cardConfigsChannel = supabase
      .channel(`card-configs-${localProfile.id}`)
      .on(
        'postgres_changes',
        {
          event: '*',
          schema: 'public',
          table: 'card_configs',
          filter: `profile_id=eq.${localProfile.id}`
        },
        (payload: any) => {
          setLocalCardConfigs(prev => {
            const current = Array.isArray(prev) ? prev : [];
            const updated = current.filter(cfg => cfg.id !== payload.new.id);
      ...
      .subscribe((status) => {
        if (status === 'SUBSCRIBED') {
          console.log('✅ Canal card-configs connecté');
        } else if (status === 'CHANNEL_ERROR') {
          console.warn('⚠️ Erreur canal card-configs');
        }
      });
    ...
    const scansChannel = supabase
      ...
      .on(
        ...
        async () => {
          ...
          try {
            const res = await fetch(`/api/analytics?profile_id=${localProfile.id}&range=all`);
    ...
    const followersChannel = supabase
      .channel(`followers-${localProfile.id}`)
      .on(
        'postgres_changes',
        {
          event: '*',
          schema: 'public',
          table: 'follows',
          filter: `followed_id=eq.${localProfile.id}`
        },
        async () => {
      ...
      .on(
        ...
        async () => {
          ...
          try {
            ...
            const hasLostCard = data?.some(card => card.status === 'lost');
            setHasLostCard(hasLostCard || false);
          } catch (err) {
    ...
    return () => {
      supabase.removeChannel(cardConfigsChannel);
...
{showSocialSection && (
  ...
  >
    ...
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
      {/* Instagram */}
      {isSectionEnabled('social', localCardConfigs) && localProfile.instagram?.trim() && (
        <SocialCard
          platform="instagram"
          label="Instagram"
          handle={localProfile.instagram.trim()}
          href={`https://instagram.com/${encodeURIComponent(localProfile.instagram.trim())}`}
      ...
      {/* LinkedIn */}
      {isSectionEnabled('social', localCardConfigs) && localProfile.linkedin?.trim() && (
        <SocialCard
          platform="linkedin"
          label="LinkedIn"
          handle={localProfile.linkedin.trim().replace(/^https?:\/\//, '').replace(/\/$/, '')}
          href={`https://linkedin.com/in/${encodeURIComponent(localProfile.linkedin.trim().replace(/^https?:\/\//, '').replace(/\/$/, ''))}`}
      ...
      {/* GitHub */}
      {isSectionEnabled('social', localCardConfigs) && localProfile.github?.trim() && (
        <SocialCard
          platform="github"
          label="GitHub"
          handle={localProfile.github.trim().replace(/^https?:\/\//, '')}
          href={`https://github.com/${encodeURIComponent(localProfile.github.trim().replace(/^https?:\/\//, ''))}`}
      ...
      {/* GitLab */}
      {isSectionEnabled('social', localCardConfigs) && localProfile.gitlab?.trim() && (
        <SocialCard
          platform="gitlab"
          label="GitLab"
          handle={localProfile.gitlab.trim().replace(/^https?:\/\//, '')}
          href={`https://gitlab.com/${encodeURIComponent(localProfile.gitlab.trim().replace(/^https?:\/\//, ''))}`}
      ...
      {/* TikTok */}
      {isSectionEnabled('social', localCardConfigs) && localProfile.tiktok?.trim() && (
        <SocialCard
          platform="tiktok"
          label="TikTok"
          handle={`@${localProfile.tiktok.trim().replace(/^@/, '')}`}
          href={`https://tiktok.com/@${encodeURIComponent(localProfile.tiktok.trim().replace(/^@/, ''))}`}
      ...
      {/* Snapchat */}
      {isSectionEnabled('social', localCardConfigs) && localProfile.snapchat?.trim() && (
        <SocialCard
          platform="snapchat"
          label="Snapchat"
          handle={`@${localProfile.snapchat.trim().replace(/^@/, '')}`}
          href={`https://snapchat.com/add/${encodeURIComponent(localProfile.snapchat.trim().replace(/^@/, ''))}`}
      ...
      {/* Telegram */}
      {isSectionEnabled('social', localCardConfigs) && localProfile.telegram?.trim() && (
        <SocialCard
          platform="telegram"
          label="Telegram"
          handle={`@${localProfile.telegram.trim().replace(/^@/, '')}`}
          href={`https://t.me/${encodeURIComponent(localProfile.telegram.trim().replace(/^@/, ''))}`}
          icon={<TelegramIcon className="w-5 h-5 text-white" />}
          gradient="from-blue-400 to-cyan-400"
        />
      )}

      {/* Behance - CORRIGÉ SANS new URL */}
{isSectionEnabled('social', localCardConfigs) && localProfile.behance?.trim() && (
  <SocialCard
    platform="behance"
    label="Behance"
    handle={cleanSocialHandle(localProfile.behance.trim(), 'behance.net')}
    href={ensureAbsoluteUrl(localProfile.behance.trim(), 'https://www.behance.net/')}
    icon={<BehanceIcon className="w-5 h-5 text-white" />}
  />
)}

{/* Dribbble - CORRIGÉ SANS new URL */}
{isSectionEnabled('social', localCardConfigs) && localProfile.dribbble?.trim() && (
  <SocialCard
    platform="dribbble"
    label="Dribbble"
    handle={cleanSocialHandle(localProfile.dribbble.trim(), 'dribbble.com')}
    href={ensureAbsoluteUrl(localProfile.dribbble.trim(), 'https://dribbble.com/')}
  ...
  />
    ...
    {[
      ...
      localProfile.gitlab,
      localProfile.tiktok,
      localProfile.snapchat,
      localProfile.telegram,
      localProfile.behance,
      localProfile.dribbble
    ].filter(v => v?.trim()).length === 0 && isSectionEnabled('social', localCardConfigs) && (
      <div className="mt-8 py-12 bg-white/5 border border-dashed border-white/10 rounded-2xl">
        <div className="text-center max-w-md mx-auto">
          <div className="w-16 h-16 mx-auto mb-4 rounded-full bg-purple-500/10 flex items-center justify-center">
            <Share2 className="w-8 h-8 text-purple-400" />
          </div>
          <h3 className="text-lg font-semibold text-white mb-2">Aucun réseau social</h3>
          <p className="text-gray-400 mb-4">
            Cette personne n'a pas encore ajouté de liens vers ses réseaux sociaux.
          </p>
        </div>
      </div>
    )}
