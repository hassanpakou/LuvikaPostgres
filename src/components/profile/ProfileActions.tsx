...
export default function FloatingButtons({ profile, setShowQRModal, onContactClick }: Props) {
  ...
  // 🔹 Gestion du scroll pour mobile uniquement
  useEffect(() => {
          ...
          >
            ...
            <motion.button
              ...
              className="w-12 h-12 rounded-full
                ...
                shadow-lg hover:bg-cyan-500/20"
          ...
          <div className="flex justify-around items-center py-3 px-2">
            {buttons.map((btn, idx) => (
              <button
                key={`mobile-${idx}`}
                onClick={btn.onClick}
                className="flex flex-col items-center gap-1 w-20 py-2 rounded-xl active:bg-white/10 transition"
              >
                <motion.div whileTap={{ scale: 0.9 }} className="text-cyan-400">
                  {btn.icon}
                </motion.div>
